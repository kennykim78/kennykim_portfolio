...
5. **데이터 추가**: `data/insights.json` 배열 **맨 앞**에 **오늘의 카테고리 쌍(2단계에서 계산한 두 카테고리) 2개 항목을 모두** 아래 형식으로 추가한다. 기존 항목 수정 금지.
   ```json
   {
     "id": "YYYY-MM-DD-영문-슬러그",
     "category": "오늘 쌍의 해당 카테고리(design·ai·plan·pm 중 하나)",
   ...
   ```
   - `id`는 날짜 + 슬러그로 유일하게. 같은 날 두 건의 슬러그가 겹치지 않게 하고, 중복이면 끝에 숫자 추가.
...
8. **IndexNow 핑 (새 글을 Bing/IndexNow에 즉시 알림)**: 커밋·푸시 후 실행한다.
   ```
   node scripts/indexnow-ping.mjs
   ```
   - Bing·Yandex 등 IndexNow 참여 엔진(ChatGPT·Copilot가 활용하는 색인)에 오늘 발행한 인사이트 URL과 홈·목록을 알린다.
   - **실패해도(네트워크 오류/비정상 응답) 발행은 성공으로 간주**하고 사유만 출력한다. 이 단계 때문에 발행을 롤백하지 않는다.

9. **완료 보고**: 발행한 2건의 제목·카테고리·URL을 각각 한 줄로 출력한다.
